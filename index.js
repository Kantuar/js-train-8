...
function copyAndSwapElements(arr, target, start, end) {
  if (target < 0 || start < 0 || end < 0 || target >= arr.length || start >= arr.length || end > arr.length || start >= end) {
...
function customIncludes(arr, element) {
  if (Array.isArray(arr)) {
    const includeElement = arr.includes(element);
